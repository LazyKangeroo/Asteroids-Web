...
function animate() {
  ...
  for (let i = wall_push.length - 1; i >= 0; i--) {
    ...
    if (
      wall.pos.x + wall.r < 0 ||
      wall.pos.x > canvas.width ||
      wall.pos.y + wall.r < 0 ||
      wall.pos.y > canvas.height
    ) {
  ...
  for (let i = asteroids.length - 1; i >= 0; i--) {
    ...
    // out of screen
    if (
      asteroid.pos.x + asteroid.r < 0 ||
      asteroid.pos.x > canvas.width ||
      asteroid.pos.y + asteroid.r < 0 ||
      asteroid.pos.y > canvas.height
    ) {
